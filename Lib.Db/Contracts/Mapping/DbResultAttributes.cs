...
#region 결과 매핑용 계약 (generated/static mapper)

/// <summary>
/// 이 특성이 부여된 타입에 대해 고성능 generated/static
/// <see cref="SqlDataReader"/> → DTO 매핑 경로를 사용할 수 있음을 표시합니다.
/// <para>
/// <b>[설계 의도]</b><br/>
/// - <b>Native AOT 지원</b>: 런타임 리플렉션 없이 정적 mapper 코드로 동작하여 AOT 호환성을 보장합니다.<br/>
/// - <b>고성능 매핑</b>: 런타임 비용(Boxing/Unboxing, Reflection)을 제거하여 Zero-Allocation에 가까운 성능을 제공합니다.
/// </para>
/// <para>
/// <b>주의:</b> generator 기반 mapper를 사용할 때는 대상 타입을 <c>partial</c>로 선언해야 합니다.
/// </para>
/// </summary>
...
/// <summary>
/// generated/static 결과 매퍼가 구현해야 하는 마커 인터페이스입니다.
/// <para>
/// <b>[설계 의도]</b><br/>
/// - <b>정적 다형성</b>: <c>static abstract</c> 메서드를 통해 인터페이스 수준에서 정적 팩터리 패턴을 강제합니다.<br/>
/// - <b>매퍼 발견</b>: 팩터리가 이 인터페이스 구현 여부를 통해 최적화된 mapper 존재를 감지합니다.
/// </para>
/// </summary>
...
public interface IMapableResult<T>
{
    ...
    /// <para>
    /// generator 기반 경로에서는 자동 생성되며, 수동 static mapper를 둘 때도 같은 계약을 따릅니다.
    /// 런타임 진단 래퍼 호환을 위해 <see cref="System.Data.Common.DbDataReader"/>
    /// overload도 함께 생성합니다.
    /// </para>
    /// </summary>
