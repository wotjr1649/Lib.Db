...
Anonymous objects and read-only DTO properties may declare stored-procedure `OUTPUT` parameters so the command can execute, but scalar values cannot be copied back into read-only members. Missing DTO properties for output-only parameters are also allowed and are ignored for copy-back. Dictionary parameter bags may receive a missing non-return output key after successful copy-back. Use a mutable DTO property, dictionary entry, `DataRow` column, or explicit `SqlParameter` when the caller needs the output value. `ReturnValue` is explicit `SqlParameter` only; DTO scalar return members are rejected in strict schema binding, and scalar `DataRow` columns named like a return value are not updated.
