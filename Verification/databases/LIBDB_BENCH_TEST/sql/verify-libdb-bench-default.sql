-- ============================================================================
-- File: verify-libdb-bench-default.sql
-- Purpose: Default benchmark database readiness checks for LIBDB_BENCH_TEST.
-- Run after: setup-libdb-bench-test.sql
-- Secret: set SQLCMDPASSWORD in the environment before running sqlcmd.
-- Run: sqlcmd -S localhost -U SA -N o -i verify-libdb-bench-default.sql -f 65001 -b
-- ============================================================================
...
IF EXISTS (SELECT 1 FROM sys.databases WHERE [name] = DB_NAME() AND [is_auto_close_on] = 1)
    INSERT INTO @Failures VALUES (N'auto-close', N'AUTO_CLOSE must be OFF for stable benchmark runs.');
