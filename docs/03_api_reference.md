...
`DataRow`는 `Output` 값을 일치하는 `DataColumn`에 반영하지만, 입력값을 함께 전달해야 하는 `OUTPUT` 파라미터와 `ReturnValue`는 셀에 명시적 `SqlParameter`를 넣어 사용하세요. `ReturnValue`는 scalar 컬럼에 쓰지 않습니다.
