...
public sealed class TvpAccessorGenerator : IIncrementalGenerator
{
        ...
        bool useDatetime2)
    {
        ...
        string sqlTypeNameLiteral = sqlTypeName is null
            ? "null"
            : GeneratorSharedHelpers.ToCSharpStringLiteral(sqlTypeName);
        ...
        static string StringLiteral(string value) => GeneratorSharedHelpers.ToCSharpStringLiteral(value);
        static string MemberIdentifier(IPropertySymbol property) => GeneratorSharedHelpers.ToCSharpIdentifier(property.Name);
        static string XmlText(string value) => GeneratorSharedHelpers.EscapeXmlText(value);
        ...
        sb.AppendLine($"    /// {XmlText(type.Name)} 타입을 위한 자동 생성 TVP 레지스트리(Extreme Runtime)입니다.");
        sb.AppendLine("    /// </summary>");
        ...
        for (int i = 0; i < props.Count; i++)
        {
            sb.AppendLine($"            cache.C{i}.Add(dto.{MemberIdentifier(props[i])});");
        }
        sb.AppendLine("        }");
        sb.AppendLine("        #endregion");
        sb.AppendLine();
        ...
        sb.AppendLine($"                {sqlTypeNameLiteral}");
        sb.AppendLine("            );");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        sb.AppendLine("            {");
        for (int i = 0; i < props.Count; i++)
            sb.AppendLine($"                {i} => dto.{MemberIdentifier(props[i])},");
        ...
        for (int i = 0; i < props.Count; i++)
        {
            ...
            sb.AppendLine($"{head} (__props[{i}] is null && string.Equals(n, {StringLiteral(props[i].Name)}, StringComparison.OrdinalIgnoreCase))");
            sb.AppendLine("                    {");
        ...
        for (int i = 0; i < props.Count; i++)
        {
            ...
            sb.AppendLine($"                            if (__props[{i}] is null && string.Equals(n, {StringLiteral(p.Name)}, StringComparison.OrdinalIgnoreCase))");
            sb.AppendLine("                            {");
            sb.AppendLine($"                                __props[{i}] = pi;");
        ...
        for (int i = 0; i < props.Count; i++)
        {
            sb.AppendLine($"            if (__props[{i}] is null) ThrowMissingProperty(typeof({fullTypeName}), {StringLiteral(props[i].Name)});");
        }
        ...
        sb.AppendLine("            {");
        for (int i = 0; i < props.Count; i++)
            sb.AppendLine($"                [{StringLiteral(props[i].Name)}] = {i},");
        ...
        sb.AppendLine("                {");
        foreach (var p in props)
            sb.AppendLine($"                    static dto => dto.{MemberIdentifier(p)},");
        sb.AppendLine("                };");
        sb.AppendLine();
        sb.AppendLine("                objectAccessors = new Func<object, object?>[__Expected]");
        sb.AppendLine("                {");
        foreach (var p in props)
            sb.AppendLine($"                    static obj => (({fullTypeName})obj).{MemberIdentifier(p)},");
        sb.AppendLine("                };");
        sb.AppendLine("            }");
        ...
        sb.AppendLine($"                SqlTypeName = {sqlTypeNameLiteral},");
        sb.AppendLine("            };");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        for (int i = 0; i < props.Count; i++)
        {
            ...
            var propertyNameLiteral = StringLiteral(p.Name);
            ...
            sb.AppendLine($"                            $\"[{i}] 불일치: '{p.Name}' 기대됨 (실제 NameHash={{col.NameHash}})\", {propertyNameLiteral}, {i});");
            ...
            sb.AppendLine($"                            $\"[{i}] '{p.Name}': 앱({expectedSqlType}) != DB({{col.SqlDbType}})\", {propertyNameLiteral}, {i});");
            sb.AppendLine("                    if (col.IsIdentity)");
            sb.AppendLine("                        throw new TvpSchemaValidationException(schema.Name, \"ID/계산컬럼_쓰기금지\",");
            sb.AppendLine($"                            $\"[{i}] '{p.Name}': Identity 컬럼은 TVP에서 값을 제공할 수 없습니다.\", {propertyNameLiteral}, {i});");
            sb.AppendLine("                    if (col.IsComputed)");
            sb.AppendLine("                        throw new TvpSchemaValidationException(schema.Name, \"ID/계산컬럼_쓰기금지\",");
            sb.AppendLine($"                            $\"[{i}] '{p.Name}': Computed 컬럼은 TVP에서 값을 제공할 수 없습니다.\", {propertyNameLiteral}, {i});");
            sb.AppendLine("                }");
            sb.AppendLine();
        }
